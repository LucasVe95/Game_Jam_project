...
public class T1FinalUI : MonoBehaviour
{
    [Header("Réglages UI")]
    ...
    public void ValiderReponse()
    {
        ...
        {
            int reel = GameManager.Instance.totalInfractionsT1;
            
            if (reponse == reel) Debug.Log("<color=green>JUSTE !</color>");
            else Debug.Log($"<color=red>FAUX ! Il y avait {reel} infractions.</color>");

            Time.timeScale = 1f;
            GameManager.menuFinalAffiche = false;
            GameManager.Instance.TerminerEtapeT1(); 
        }
    }
}